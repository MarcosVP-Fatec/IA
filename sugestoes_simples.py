...
def incluir_regras():
    while True:
        rule_db = ler_bd()
        novaregra = input("Informe o que foi comprado para sugerido para nova regra: ").replace(" ","").lower()
        if novaregra == "":
            break
        ...
        if jah_existe(rule_db, novaregra):
            ...
            copc = input("<Enter> Voltar | X-Excluir: ")
            if copc == 'X' or copc == 'x':
                cur = mariadb_connect_cursor()
            ...
            novasugestao = input("Informe a sugestão: ").lower()
            if novasugestao != "":
                cur = mariadb_connect_cursor()
...
def listar_compra():
    for item in rule_db:
        print("Se você comprar (" + item["percept"] + ")")
        print("         sugira (" + item["action"] + ")\n")

###################################################################
# Verificando o que irá rodar
while True:
    opc = input("Selecione 1-Incluir/Excluir Regras | 2-Informar compra | 3-Listar: ")
    if opc == '1':
        incluir_regras()
        rule_db = ler_bd()
    elif opc == '2':
